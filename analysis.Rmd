...
To address a point raised by reviewer 2: *What is more convincing though is that the difference in the proportion of deaths both directly and indirectly attributable to breast cancer in the screen detected versus symptomatic patients looks considerably greater than the difference in unrelated deaths between the two groups, which if anything is more supportive of screening reducing mortality,*

I believe they are referring to over 71, so we focus on this group.
...
myKable(bc_outcome, caption="Cancer deaths vs unrelated deaths, age > 71")

fisher.test(bc_outcome %>% select(count_bc_death, count_unrelated_death))
```

We see that in the 71+ age group proportion of breast cancer deaths (direct and indirect) is reduced from 42% for symptomatic patients to 10% for screening patients. At the same time unrelated deaths are 22% and 14%, respectively. The difference between these two proportions is strongly significant ($p=3\times10^{-9}$, Fisher's test).

Just for comparison, the same proportions, but for all age groups:

```{r cancer_unrelated_deaths}
bc_outcome <- sdat %>%
  death_propotions()

myKable(bc_outcome, caption="Cancer deaths vs unrelated deaths, all age groups")
```
...
g1 <- plotGroupAge(d71, what="proportion", x="Outcome", group="Diagnostic") + theme(axis.text.x = element_text(angle=40, hjust=1)) + ggtitle("71+") + theme(legend.position = "none") + labs(y="Proportion")
g2 <- sdat %>% filter(Midage < 71) %>% groupAge %>% plotGroupAge(what="proportion", x="Outcome", group="Diagnostic") + theme(axis.text.x = element_text(angle=40, hjust=1)) + ggtitle("26-70") + labs(y=NULL, fill="Diagnostic route")
