...
```{r fit_funs}
fitData <- function(d, value, others) {
  d %>% 
    findOutcomeProportion(value, others) %>% 
...
plotFitData <- function(d, value, others, tit=NULL) {
  if(is.null(tit)) tit <- paste(value, "proportion")
  
  dat <- d %>% 
    findOutcomeProportion(value, others)
  mod <- fitData(d, value, others)
  
  ggplot(data=dat, aes(x=Midage, y=p, group=Diagnostic, colour = Diagnostic)) +
    theme_linedraw() +
    ...
    scale_colour_manual(values = cbPalette) +
    scale_y_continuous(limits=c(0,1)) +
    geom_line(data=mod, aes(x=x, y=y, colour=Diagnostic))
}
```
