...
plotGroupAge <- function(d, what="count", pos="dodge") {
  d <- d %>% mutate(y = if(what == "count") n else p)
  ...
  g <- ggplot(d, aes(x=Diagnostic, y=y, group=Outcome, fill=Outcome)) +
...
The unrelated death proportion is greater in the symptomatic patients. The difference is statistically significant, $p = `r signif(ft$p.value, 2)`$.

Below I repeat the above calculations for each age group (above 71) separately. As we can see, the difference in unrelated deaths is less convincing per group (overlapping error bars), only becomes pronounced when they are pooled. Also, the last figure in the "Alternative visualization" section (above) shows very little difference between the two diagnostics, when all ages trend is taken into account.

```{r age_comparison, fig.width=10, fig.height=3}
ages <- c("71-75", "76-80", "81-85", "86+")
map(ages, function(age) {
  sdat %>% 
    filter(Age == age) %>% 
    groupAge %>% 
    plotGroupAge(what="proportion") +
      ggtitle(age) +
...
timeCourse <- function(d, value, others, min.age=35, tit=NULL, with.half=FALSE) {
  # fit time course with a linear function, using nls to include errors
  coef <- timeCourseFit(d, value, others, min.age=35)
  b0 <- coef$Estimate[1]
  b1 <- coef$Estimate[2]

  # find half time and its error, using inverse linear fit
  dx <- d %>%  
    findOutcomeProportion(value, others) %>% 
    select(x=Midage, y=p, se)
  f12 <- lm(x ~ y, data=dx)
  p12 <- predict(f12, tibble(y=0.5), se.fit = TRUE, level=0.95)
  
  halfx <- p12$fit
  halfx.err <- p12$se.fit
  
  xx <- c(min.age, 87.5)
  mod <- tibble(x = xx, y = b0 + xx * b1)
  ...
  if(with.half) {
    g <- g + 
      ...
      geom_vline(xintercept = halfx, linetype = "dotted", colour="grey50")
  }
  list(plot=g, coef=coef, half=halfx, half.err=halfx.err)
}
```
