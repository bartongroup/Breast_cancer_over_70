...
source("../R/calc.R")
source("../R/plots.R")
tar_load(c(summ_sym, summ_screen, dbc_logit))
#g1 <- plotDeathProp(d_both) + labs(title="All patients")
g2 <- plot_death_prop(summ_sym, mod=dbc_logit %>% mutate(x=Midage, y=prob) %>% filter(Diagnostic == "symptomatic")) + labs(title="Symptomatic patients")
g3 <- plot_death_prop(summ_screen, mod=dbc_logit %>% mutate(x=Midage, y=prob) %>% filter(Diagnostic == "screening")) + labs(title="Screened patients", y=NULL)
g <- cowplot::plot_grid(g2, g3, nrow=1, align="h")
g

file <- "death_proportion.pdf"
link_death <- paste0(pdf_url, file)
ggsave(file.path(pdf_dir, file), plot = g, device = "pdf", width=10, height=3)
```
...
g1 <- set$summ %>% filter(Midage < 75) %>% group_age() %>% plot_group_age(what="proportion", x="Outcome", group="Diagnostic", ymax=1) + theme(axis.text.x = element_text(angle=40, hjust=1)) + ggtitle("26-75") + theme(legend.position = "none") +  labs(y="Proportion", fill="Diagnostic route")

g2 <- set$summ %>% filter(Midage > 75) %>% group_age() %>% plot_group_age(what="proportion", x="Outcome", group="Diagnostic", ymax=1) + theme(axis.text.x = element_text(angle=40, hjust=1)) + ggtitle("76+") + labs(y=NULL)



g <- cowplot::plot_grid(g1, g2, nrow=1, align = "h", rel_widths = c(1,1.3))
g
